...
export const acceptOrder = async (req, res) => {
 try {
  ...
  res.json({ message: "Order accepted (marked completed) successfully.", order });
 } catch (error) {
...
export const rejectOrder = async (req, res) => {
 try {
  ...
  if (!order) return res.status(404).json({ message: "Order not found." });
  res.json({ message: "Order rejected (marked cancelled) successfully.", order });
 } catch (error) {
...
export const getPendingReservations = async (req, res) => {
 try {
  ...
  const reservations = await Reservation.find({ status: 'pending' }).sort({ date: 1, time: 1 });
...
export const declineReservation = async (req, res) => {
 try {
  ...
  if (!reservation) return res.status(404).json({ message: "Reservation not found." });
  res.json({ message: "Reservation declined (marked cancelled) successfully.", reservation });
 } catch (error) {
